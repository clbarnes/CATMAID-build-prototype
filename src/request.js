...
var RequestQueue = function(originUrl, csrfToken)
{
  ...
  var send = function()
  {
    RequestQueue.trigger(CATMAID_RequestQueue.EVENT_REQUEST_STARTED, self);
    var item = queue[0];
    xmlHttp.open(
      item.method,
      item.request,
  ...
  var callback = function()
  {
    ...
    {
      var advance = true;
      RequestQueue.trigger(CATMAID_RequestQueue.EVENT_REQUEST_ENDED, self);
      try {
        // Throw exception in case of a network error
        if (xmlHttp.status === 0) {
          throw new CATMAID_NetworkAccessError("Network unreachable",
    ...
    )
    {
      ...
      for ( var i = 1; i < queue.length; ++i )
      {
        if ( queue[ i ].id == id )
        {
          ...
          CATMAID_statusBar.replaceLast( "replacing request ", + r );
...
CATMAID_asEventSource(RequestQueue);
