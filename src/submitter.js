 ...
 *
 * var submit = CATMAID_submitterFn();
 *
 * ... then submit requests like:
 *
 * submit(CATMAID_makeURL('/skeleton_id/' + skeleton_id),
 *    'POST',
 *    {all: true},
...
export let CATMAID_submitterFn = function() {
  ...
  var reset = function(q, error) {
    ...
    if (!handled) {
      ...
      console.log(error, q);
      if (!q.quiet) {
        var err = (error && error.error) ? CATMAID_parseErrorResponse(error) :
          new CATMAID_Error('An unknown error occured');
        CATMAID_handleError(err);
      }
    }

    // If there was an error in one of the callbacks, report this as well.
    if (callbackError) {
      CATMAID_error(callbackError);
    }
  };
