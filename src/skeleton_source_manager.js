...
CATMAID_asEventSource(SkeletonSourceManager.prototype);
...
SkeletonSourceManager.prototype.destroy = function() {
  CATMAID_Skeletons.off(CATMAID_Skeletons.EVENT_SKELETONS_JOINED,
      this.replaceSkeleton);
  CATMAID_Skeletons.off(CATMAID_Skeletons.EVENT_SKELETON_DELETED,
      this.removeSkeleton);
...
SkeletonSourceManager.prototype.getSubscriptionExpression = function(source) {
  ...
  if (subscriptions && subscriptions.length > 0) {
    ...
    return 'Widget skeletons = ' + subscriptions.reduce(function(o, s, i) {
      var name = 'S' + (i + 1);
      if (0 === i) {
        var union = CATMAID_SkeletonSource.operations[CATMAID_SkeletonSource.UNION];
        return source.ignoreLocal ? name : ('(local ' + union + ' ' + name + ')');
      } else {
        return '(' + o + ' ' + CATMAID_SkeletonSource.operations[s.op] + ' ' + name + ')';
      }
    }, undefined);
...
SkeletonSourceManager.prototype.createSourceControls = function(source, options) {
  ...
  */
  ...
  var createOpSelector = function(value) {
    ...
    opSelect.options.add(new Option('Union', CATMAID_SkeletonSource.UNION));
    opSelect.options.add(new Option('Intersection', CATMAID_SkeletonSource.INTERSECTION));
    opSelect.options.add(new Option('Difference', CATMAID_SkeletonSource.DIFFERENCE));
    var defaultValue = (undefined === value) ? CATMAID_SkeletonSource.UNION : value;
  ...
  help.href = CATMAID_makeDocURL('user_faq.html#faq-source-subscriptions');
  help.target = '_blank';
  ...
  subscribe.onclick = (function(e) {
    ...
    if (fromSource) {
      ...
      */
      ...
      var subscription = new CATMAID_SkeletonSourceSubscription(
      ...
      try {
        ...
        if (error instanceof CATMAID_SubscriptionError) {
          CATMAID_warn(error.message);
        } else {
...
SkeletonSourceManager.prototype.getSelectedSkeletons = function(ref_source, silent) {
  ...
  if (0 === skeletons.length && !silent) {
    CATMAID_info('No skeletons available at ' + source.getName());
  }
  return skeletons;
};
...
SkeletonSourceManager.prototype.getSelectedSkeletonModels = function(ref_source) {
  ...
  if (0 === models.length) {
    CATMAID_info('No skeletons selected at ' + source.getName());
  }
  return models;
};
...
CATMAID_SkeletonSourceManager = SkeletonSourceManager;
