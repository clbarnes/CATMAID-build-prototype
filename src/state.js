...
var get_or_error = function(obj, field) {
  ...
  throw new CATMAID_ValueError("Couldn't read field \"" +
...
GenericState.prototype.makeNodeState = function(nodeId) {
  ...
  if (!node) {
    throw new CATMAID_ValueError("Couldn't find node " + nodeId + " in state");
  }
  return CATMAID_getNodeState(node[0], node[1]);
};
...
GenericState.prototype.makeParentState = function(nodeId) {
  ...
  return CATMAID_getParentState(parent[0], parent[1]);
};
...
GenericState.prototype.makeLocalState = function(options) {
  ...
  if (options.nodeId) {
    ...
    if (!node) {
      throw new CATMAID_ValueError("Couldn't find node node " +
  ...
  if (options.parentId) {
    ...
    if (!parent) {
      throw new CATMAID_ValueError("Couldn't find parent node " +
  ...
  if (options.childIds && options.childIds.length > 0) {
    ...
    for (var i=0; i<options.childIds.length; ++i) {
      ...
      if (!child) {
        throw new CATMAID_ValueError("Couldn't find child node " + childId + " in state");
      }
      children.push(child);
    }
    state['children'] = children;
  ...
  if (options.links && options.links.length > 0) {
    ...
    for (var i=0; i<options.links.length; ++i) {
      ...
      if (!link) {
        throw new CATMAID_ValueError("Couldn't find link " + link + " in state");
      }
    }
...
GenericState.prototype.makeEdgeState = function(nodeId, parentId) {
  ...
  if (!parent) {
    throw new CATMAID_ValueError("Couldn't find parent node " + nodeId + " in state");
  }
  return CATMAID_getEdgeState(parent[0], parent[1], node[0], node[1]);
};
...
GenericState.prototype.makeNeighborhoodState = function(nodeId, isConnector) {
  ...
  if (isConnector) {
    ...
    return CATMAID_getNeighborhoodState(node[0], node[1], null, null, null, null, cLinks);
  } else {
    var parent = this.getParent(nodeId);
    if (!parent) {
      throw new CATMAID_ValueError("Couldn't find parent of node " + nodeId + " in state");
    }
    var links = this.getLinks(nodeId, false);
    return CATMAID_getNeighborhoodState(node[0], node[1], parent[0], parent[1],
...
SimpleSetState.prototype.getLinks = CATMAID_noop;
SimpleSetState.prototype.getParent = CATMAID_noop;
SimpleSetState.prototype.getChildren = CATMAID_noop;
...
LocalState.prototype.makeParentState = function(nodeId) {
  if (!this.parent) {
    throw new CATMAID_ValueError("Couldn't find node " + nodeId + " to create parent state");
  }
  return CATMAID_getParentState(this.parent[0], this.parent[1]);
};

LocalState.prototype.makeEdgeState = function(childId, parentId) {
  if (!this.node || this.node[0] != parentId) {
    throw new CATMAID_ValueError("Couldn't find parent node " + parentId +
        " to create edge state");
  }
  if (!this.children || 0 === this.children.length) {
    throw new CATMAID_ValueError("Couldn't find child node " + childId +
        " to create edge state");
  }
  var child;
  for (var i=0; i<this.children.length; ++i) {
  ...
  return CATMAID_getEdgeState(this.node[0], this.node[1], child[0], child[1]);
};
...
NoCheckState.prototype.makeNodeState = CATMAID_getNoCheckState;
NoCheckState.prototype.makeParentState = CATMAID_getNoCheckState;
NoCheckState.prototype.makeEdgeState = CATMAID_getNoCheckState;
NoCheckState.prototype.makeNeighborhoodState = CATMAID_getNoCheckState;
