...
ConnectivityMatrix.prototype.refresh = function() {
  ...
  return CATMAID_fetch(project.id + '/skeleton/connectivity_matrix', 'POST', {
...
ConnectivityMatrix.prototype.filterResults = function(data) {
  var hasResults = data && !CATMAID_tools.isEmpty(data);
  if (this.filterRules.length > 0 && this.applyFilterRules && hasResults) {
    ...
    for (var sourceSkeletonId in data) {
      ...
      for (var targetSkeletonId in targets) {
        ...
        if (!links.locations) {
          throw new CATMAID_ValueError("Could not find location information in connectivity matrix");
        }
        for (var connectorId in links.locations) {
          ...
          if (!connectors.has(connectorId)) {
            ...
            connectors.set(connectorId,
                new CATMAID_ConnectorModel(connectorId, x, y, z));
          }
        }
      }
    }
    // Filter connectors
    var filter = new CATMAID_NodeFilter(this.filterRules, connectors);
    return filter.execute()
      .then(function(filtered) {
        if (filtered.nodes.size === 0) {
          CATMAID_warn("No connectors left after filter application");
        ...
        for (var sourceSkeletonId in data) {
          ...
          if (CATMAID_tools.isEmpty(targets)) {
