...
CATMAID_Volume.prototype = {};
CATMAID_asEventSource(CATMAID_Volume.prototype);

CATMAID_Volume.prototype.EVENT_PROPERTY_CHANGED = "volume_property_changed";
 ...
 */
CATMAID_Volume.prototype.set = function(field, value, forceOverride) {
 ...
 */
CATMAID_Volume.prototype.save = function() {
  if (null === this.id) {
    return CATMAID_Volumes.add(project.id, this.serialize())
      .then(function(result) {
        if (result.success) {
          CATMAID_msg("Success", "A new volume was created");
        } else {
          CATMAID_warn("Unknown status");
        }
      });
  } else {
    return CATMAID_Volumes.update(project.id, this.id, this.serialize())
      .then(function(result) {
        if (result.success) {
          CATMAID_msg("Changes saved", "The volume has been udpated");
        } else {
          CATMAID_warn("Unknown status");
        }
      });
...
export let CATMAID_BoxVolume = function(options) {
  ...
  this.set("preview", CATMAID_tools.getDefined(options.preview, true));
  this.set("previewColor", CATMAID_tools.getDefined(options.previewColor, '#FFFFFF'));
  this.set("previewOpacity", CATMAID_tools.getDefined(options.previewOpacity, 0.7));
  this.meshNeedsSync = true;
  this._previewMeshManager = null;
};

CATMAID_BoxVolume.prototype = Object.create(CATMAID_Volume.prototype);
CATMAID_BoxVolume.prototype.constructor = CATMAID_BoxVolume;

CATMAID_BoxVolume.prototype.init = function() {
  this.updateTriangleMesh();
  return this;
};

CATMAID_BoxVolume.prototype.clearPreviewData = function() {
  // If there is an existing removal function, call it.
  if (this._previewMeshManager) {
    CATMAID_tools.callIfFn(this._previewMeshManager.remove());
  }
};

CATMAID_BoxVolume.prototype.updateTriangleMesh = function(onSuccess, onError) {
  ...
  if (this.preview) {
    work = work
      .then(mesh =>
        CATMAID_BoxVolume.showCompartment(mesh, this.previewColor,
        this.previewOpacity))
      .then(meshInfo => {
  ...
  return work
    .then(mesh => {
      ...
      this.set("mesh", mesh);
      if (CATMAID_tools.isFn(onSuccess)) {
        onSuccess(this, mesh);
      }
      return mesh;
    })
...
CATMAID_BoxVolume.prototype.createMesh = function() {
  let points = [
 ...
 */
CATMAID_BoxVolume.showCompartment = function(mesh, color, opacity) {
  var list = mesh ? [mesh] : [];
  var meshManager = CATMAID_BoxVolume.showMeshesIn3DViewer(
      list, color, opacity);
 ...
 */
CATMAID_BoxVolume.showMeshesIn3DViewer = function(meshes, color, opacity) {
  var w = CATMAID_WebGLApplication.prototype.getFirstInstance();
  if (!w || !w.space) {
...
CATMAID_BoxVolume.prototype.set = function(field, value, forceOverride, noSyncCheck) {
  ...
  CATMAID_Volume.prototype.set.call(this, field, value, forceOverride);
 ...
 */
CATMAID_BoxVolume.prototype.serialize = function() {
  return {
    min_x: this.minX,
    min_y: this.minY,
    min_z: this.minZ,
...
CATMAID_ConvexHullVolume.prototype = Object.create(CATMAID_Volume.prototype);
CATMAID_ConvexHullVolume.prototype.constructor = CATMAID_ConvexHullVolume;

CATMAID_ConvexHullVolume.prototype.init = function() {
  this.updateTriangleMesh();
  return this;
};
 ...
 * be updated.
 */
CATMAID_ConvexHullVolume.prototype.set = function(field, value, forceOverride, noSyncCheck) {
  // Check parameter for influence on mesh *before* the prototype is called,
  // this makes sure all property change event handlers can already know that
  // the mesh needs to be updated. In case a mesh set directly no sync is
  // needed anymore.
 ...
 */
CATMAID_ConvexHullVolume.prototype.updateTriangleMesh = function(onSuccess, onError) {
  ...
  if (this.preview) {
    CATMAID_ConvexHullVolume.showCompartment(skeletons, rules, this.respectRadius,
        this.previewColor, this.previewOpacity, createMesh, update);
  } else {
    CATMAID_ConvexHullVolume.createTriangleMesh(skeletons, rules, this.respectRadius,
        createMesh, update);
  }
};
 ...
 */
CATMAID_ConvexHullVolume.prototype.clearPreviewData = function() {
  // If there is an existing removal function, call it.
  if (this._previewMeshManager) {
    CATMAID_tools.callIfFn(this._previewMeshManager.remove());
  }
...
var createCompartments = function(skeletons, compartments, skeleton_arbors,
  ...
  return Promise.all(Object.keys(compartments).map(function(name) {
      ...
      var filter = new CATMAID_SkeletonFilter(rules, skeletons);
 ...
 */
CATMAID_ConvexHullVolume.prototype.createMesh = function(points) {
 ...
 * neuron source. The current filter rules are taken into account.
 */
CATMAID_ConvexHullVolume.createTriangleMeshes = function(skeletons, compartments,
  ...
  CATMAID_SkeletonFilter.fetchArbors(Object.keys(skeletons), true, true, true)
 ...
 */
CATMAID_ConvexHullVolume.createTriangleMesh = function(skeletons, rules,
    respectRadius, createMesh, onSuccess) {
  var name = 'compartment';
  var compartments = {};
  compartments[name] = rules;
  CATMAID_ConvexHullVolume.createTriangleMeshes(skeletons, compartments,
 ...
 * operate on the addeded meshes.
 */
CATMAID_ConvexHullVolume.showMeshesIn3DViewer = function(meshes, color, opacity) {
  var w = CATMAID_WebGLApplication.prototype.getFirstInstance();
  if (!w || !w.space) {
    // Silently fail if no 3D viewer is open
    return;
 ...
 */
CATMAID_ConvexHullVolume.showCompartments = function(skeletons, compartments,
    respectRadius, color, opacity, createMesh, onSuccess) {
  CATMAID_ConvexHullVolume.createTriangleMeshes(skeletons, compartments,
      createMesh, respectRadius,
      function(meshes) {
        var meshManager = CATMAID_ConvexHullVolume.showMeshesIn3DViewer(
 ...
 * Create a convex hull and display it in the first available 3D viewer.
 */
CATMAID_ConvexHullVolume.showCompartment = function(skeletons, rules, respectRadius,
    color, opacity, createMesh, onSuccess) {
  CATMAID_ConvexHullVolume.createTriangleMesh(skeletons, rules,
 ...
 * Get a JSON representation of this object.
 */
CATMAID_ConvexHullVolume.prototype.serialize = function() {
...
export let CATMAID_AlphaShapeVolume = function(options) {
  ...
  CATMAID_ConvexHullVolume.call(this, options);
...
CATMAID_AlphaShapeVolume.prototype = Object.create(CATMAID_ConvexHullVolume.prototype);
CATMAID_AlphaShapeVolume.prototype.constructor = CATMAID_AlphaShapeVolume;

CATMAID_AlphaShapeVolume.prototype.init = function() {
  this.updateTriangleMesh();
  return this;
};
 ...
 * be updated.
 */
CATMAID_AlphaShapeVolume.prototype.set = function(field, value, forceOverride) {
  ...
  CATMAID_ConvexHullVolume.prototype.set.call(this, field, value, forceOverride,
      refreshMesh);
  ...
  if (refreshMesh) {
    ...
    if (this.preview) {
      ...
      this._previewMeshManager = CATMAID_ConvexHullVolume.showMeshesIn3DViewer(
 ...
 */
CATMAID_AlphaShapeVolume.prototype.filterMesh = function() {
 ...
 * it easier for sub-types to override.
 */
CATMAID_AlphaShapeVolume.prototype.createMesh = function(points) {
  // Don't compute 2-simplices if no triangles are needed (i.e. tetrahedra
  // will be looked at.
  var lowestSimplexK = this.filterTriangles ? 2 : 3;
  this.intervalMesh = CATMAID_alphaIntervalComplex(points, lowestSimplexK);
...
export let CATMAID_alphaIntervalComplex = function(points, minD) {
  ...
  var sphereIsEmpty = CATMAID_sphereIsEmpty;
