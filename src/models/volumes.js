...
var Volumes = {
  ...
  listAll: function(projectId, skeletonIds) {
    ...
    return CATMAID_fetch(url, method, {
      'skeleton_ids': skeletonIds,
    }).then(function (volumes) {
      return volumes.data.map(function (vol) {
        return CATMAID_tools.buildObject(volumes.columns, vol);
      });
    });
  },
  ...
  update: function(projectId, volumeId, serializedVolume) {
    ...
    return CATMAID_fetch(url, "POST", serializedVolume).then(function(json) {
      return json;
    });
  },
  ...
  add: function(projectId, serializedVolume) {
    ...
    return CATMAID_fetch(url, "POST", serializedVolume).then(function(json) {
      CATMAID_Volumes.trigger(CATMAID_Volumes.EVENT_VOLUME_ADDED, json.volume_id);
      return json;
    });
  },
  ...
  x3dToMeshes: function(x3d) {
    var vrml = CATMAID_Volumes.x3dToVrml(x3d);
  ...
  findSkeletonInnervations: function(projectId, skeletonIds, annotation) {
    return CATMAID_fetch(projectId + '/volumes/skeleton-innervations', 'POST', {
      'skeleton_ids': skeletonIds,
  ...
  intersectsBoundingBox: function(projectId, volumeId, x, y, z) {
    ...
    return CATMAID_fetch(url, "GET", {x: x, y: y, z: z});
  },
};

// Add events
CATMAID_asEventSource(Volumes);
