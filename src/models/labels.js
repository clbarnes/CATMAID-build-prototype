...
var Labels = {
  ...
  listAllDetail: function(projectId) {
    return CATMAID_fetch(projectId + '/labels/detail', 'GET');
  },
  ...
  update: function(projectId, nodeId, nodeType, newLabels, deleteExisting) {
    ...
    return CATMAID_fetch(url, 'POST', params).then(function(json) {
      CATMAID_Labels.trigger(CATMAID_Labels.EVENT_NODE_LABELS_CHANGED, nodeId);
      if (json.warning) {
        CATMAID_warn(json.warning);
      }
      return {
  ...
  remove: function(projectId, nodeId, nodeType, label) {
    ...
    return CATMAID_fetch(url, 'POST', {tag: label}).then(function(json) {
      CATMAID_Labels.trigger(CATMAID_Labels.EVENT_NODE_LABELS_CHANGED, nodeId);
      return {
        'deletedLabels': [label],
      };
...
CATMAID_AddTagsToNodeCommand = CATMAID_makeCommand(function(projectId, nodeId, nodeType,
  ...
  var exec = function(done, command) {
    var addLabel = CATMAID_Labels.update(projectId, nodeId, nodeType,
        tags, deleteExisting);
  ...
  var undo = function(done, command) {
    ...
    if (undefined === command._addedTags) {
      throw new CATMAID_ValueError('Can\'t undo creation of tag, original data not available');
    }
    ...
    var removeLabel = 0 === command._addedTags.length ? Promise.resolve() :
      Promise.all(command._addedTags.map(function(t) {
        return CATMAID_Labels.remove(projectId, nodeId, nodeType, t);
      }));
...
CATMAID_RemoveTagFromNodeCommand = CATMAID_makeCommand(function(projectId, nodeId,
  ...
  var exec = function(done, command) {
    var removeLabel = CATMAID_Labels.remove(projectId, nodeId, nodeType, tag);
  ...
  var undo = function(done, command) {
    ...
    if (undefined === command._deletedLabels) {
      throw new CATMAID_ValueError('Can\'t undo deletion of tag, history data not available');
    }
    ...
    var addLabel = (command._deletedLabels.length === 0) ? Promise.resolve() :
        CATMAID_Labels.update(projectId, nodeId, nodeType, command._deletedLabels);
