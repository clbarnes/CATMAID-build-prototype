...
var Skeletons = {
  ...
  split: function(state, projectId, treenodeId,
    ...
    return CATMAID_fetch(url, 'POST', params).then((function(json) {
      this.trigger(CATMAID_Skeletons.EVENT_SKELETON_SPLIT,
          json.new_skeleton_id,
          json.existing_skeleton_id,
          treenodeId);
      this.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
  ...
  join: function(state, projectId, fromId, toId, annotationSet, samplerHandling) {
    ...
    return CATMAID_fetch(url, 'POST', params).then((function(json) {
      ...
      CATMAID_Skeletons.trigger(
          CATMAID_Skeletons.EVENT_SKELETONS_JOINED, json.deleted_skeleton_id,
              json.result_skeleton_id);
      CATMAID_Skeletons.trigger(
          CATMAID_Skeletons.EVENT_SKELETON_DELETED, json.deleted_skeleton_id);
      CATMAID_Skeletons.trigger(
          CATMAID_Skeletons.EVENT_SKELETON_CHANGED, json.result_skeleton_id);
      return json;
    }).bind(this));
  },
  ...
  reroot: function(state, projectID, treenodeID) {
    ...
    return CATMAID_fetch(url, 'POST', params).then((function(json) {
      this.trigger(CATMAID_Skeletons.EVENT_SKELETON_REROOTED,
          json.skeleton_id,
          treenodeID);
      this.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
          json.skeleton_id);
  ...
  withLabels: function(projectID, labelIds, labelNames) {
    return CATMAID_fetch(project.id + '/skeletons/node-labels', 'POST', {
      label_ids: labelIds,
  ...
  getSWC: function(projectId, skeletonIds, linearizeIds, somaMarkers) {
    ...
    var swcRequests = skeletonIds.map(function(skid) {
      return CATMAID_fetch(projectId + '/skeleton/' + skid + '/swc', 'GET', {
  ...
  exportSWC: function(projectId, skeletonIds, linearizeIds, archive,
      somaMarkers) {
    return CATMAID_Skeletons.getSWC(projectId, skeletonIds, linearizeIds,
        somaMarkers)
  ...
  exportNRRD: function(projectId, skeletonId, mirror, sourceSpace, targetSpace, asyncProc) {
    return CATMAID_fetch(projectId + '/skeleton/' + skeletonId + '/nrrd', 'POST', {
      mirror: mirror,
  ...
  getRootNode: function(projectId, skeletonId) {
    return CATMAID_fetch(project.id + '/skeletons/' + skeletonId + '/root');
  },
  ...
  inBoundingBox: function(projectId, minX, minY, minZ, maxX, maxY, maxZ,
    ...
    return CATMAID_fetch(project.id + '/skeletons/in-bounding-box', method, {
      'minx': minX,
      'miny': minY,
      'minz': minZ,
      'maxx': maxX,
  ...
  getSamplerCount: function(projectId, skeletonId) {
    return CATMAID_fetch(projectId + '/skeletons/' + skeletonId + '/sampler-count')
      .then(function(count) {
  ...
  getAllSamplerCounts: function(projectId, skeletonIds) {
    return CATMAID_fetch(projectId + '/skeletons/sampler-count', 'POST', {
      'skeleton_ids': skeletonIds,
    });
  },
  ...
  distanceBetweenNodes: function(projectId, skeletonId, nodeA, nodeB, arborTransform) {
    ...
    return CATMAID_fetch(projectId + '/skeletons/' + skeletonId + '/compact-detail')
      .then(function(skeletonDetail) {
        let arborParser = new CATMAID_ArborParser();
        arborParser.init('compact-skeleton', skeletonDetail);
        if (CATMAID_tools.isFn(arborTransform)) {
          arborTransform(arborParser);
        }

         // Make sure, nodes A and B are actually part of the skeleton.
         if (!arborParser.positions[nodeA]) {
           throw new CATMAID_ValueError("Node " + nodeA + " is not part of skeleton " + skeletonId);
         }
         if (!arborParser.positions[nodeB]) {
           throw new CATMAID_ValueError("Node " + nodeB + " is not part of skeleton " + skeletonId);
         }
...
CATMAID_SplitSkeletonCommand = CATMAID_makeCommand(
  ...
  var exec = function(done, command, map) {
    var split = CATMAID_Skeletons.split(state,
  ...
  var undo = function(done, command, map) {
    throw new CATMAID_ValueError("Undo of skeleton splits is not allowed at the moment");
  };
...
CATMAID_JoinSkeletonsCommand = CATMAID_makeCommand(
  ...
  var exec = function(done, command, map) {
    var join = CATMAID_Skeletons.join(state, project.id, fromId, toId,
  ...
  var undo = function(done, command, map) {
    throw new CATMAID_ValueError("Undo of skeleton joins is not allowed at the moment");
  };
...
CATMAID_RerootSkeletonCommand = CATMAID_makeCommand(
  ...
  var exec = function(done, command, map) {
    var reroot = CATMAID_Skeletons.reroot(state, projectID, treenodeID);
  ...
  var undo = function(done, command, map) {
    throw new CATMAID_ValueError("Undo of skeleton rerooting is not allowed at the moment");
  };
