...
var Nodes = {
  ...
  updateRadius: function(state, projectId, nodeId, radius, updateMode) {
    ...
    return CATMAID_fetch(url, 'POST', params).then((function(json) {
      this.trigger(CATMAID_Nodes.EVENT_NODE_RADIUS_CHANGED, json.updated_nodes);
      return {
  ...
  updateConfidence: function(state, projectId, nodeId, newConfidence,
    ...
    return CATMAID_fetch(url, 'POST', params)
      .then((function(result) {
        this.trigger(CATMAID_Nodes.EVENT_NODE_CONFIDENCE_CHANGED, nodeId,
  ...
  updateParent: function(projectId, nodeId, newParentId) {
    ...
    return CATMAID_fetch(url, 'POST', params)
      .then(function(result) {
        CATMAID_Skeletons.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
            result.skeleton_id);
        return result;
      });
  },
  ...
  create: function(state, projectId, x, y, z, parentId, radius, confidence,
    ...
    return CATMAID_fetch(url, 'POST', params)
      .then(function(result) {
        var newNode = new CATMAID_Treenode(result.treenode_id, x, y, z,
            parentId, undefined, result.skeleton_id, radius,
            confidence, CATMAID_session.userid, result.edition_time);
        CATMAID_Nodes.trigger(CATMAID_Nodes.EVENT_NODE_CREATED, newNode);
        CATMAID_Skeletons.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
    ...
    return CATMAID_fetch(url, 'POST', params)
      .then(function(result) {
        var newNode = new CATMAID_Treenode(result.treenode_id, x, y, z,
            ...
            radius, confidence, CATMAID_session.userid, result.edition_time);
        CATMAID_Nodes.trigger(CATMAID_Nodes.EVENT_NODE_CREATED, newNode);
        CATMAID_Skeletons.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
            result.skeleton_id, [[result.treenode_id, x, y, z]]);
        return result;
  ...
  remove: function(state, projectId, nodeId) {
    ...
    return CATMAID_fetch(url, 'POST', params)
      .then(function(result) {
        CATMAID_Nodes.trigger(CATMAID_Nodes.EVENT_NODE_DELETED, nodeId,
            result.parent_id);
        ...
        if (result.deleted_neuron) {
          CATMAID_Skeletons.trigger(CATMAID_Skeletons.EVENT_SKELETON_DELETED,
              result.skeleton_id);
        } else {
          CATMAID_Skeletons.trigger(CATMAID_Skeletons.EVENT_SKELETON_CHANGED,
  ...
  getLocation: function(nodeId) {
    return CATMAID_fetch(project.id + "/node/get_location",
        'POST', {tnid: nodeId});
  },
  ...
  getSuppressdVirtualNodes: function(projectId, nodeId) {
    return CATMAID_fetch(project.id + "/treenodes/" + nodeId + "/suppressed-virtual/");
  },
  ...
  deleteSuppresedVirtualNode: function(projectId, nodeId, suppressedId) {
    ...
    return CATMAID_fetch(url, 'DELETE');
  },

};

function announceNodeUpdate(node) {
  CATMAID_Nodes.trigger(CATMAID_Nodes.EVENT_NODE_UPDATED, node[0]);
}
...
CATMAID_UpdateNodeRadiusCommand = CATMAID_makeCommand(function(
  ...
  var exec = function(done, command, map) {
    ...
    var execState = new CATMAID_LocalState([mNode.value, mNode.timestamp]);
    var updateRadius = CATMAID_Nodes.updateRadius(execState, projectId, nodeId,
        radius, updateMode);
  ...
  var undo = function(done, command, map) {
    ...
    var undoState = new CATMAID_SimpleSetState(mappedNodes.state);
    var updateRadii = CATMAID_Nodes.updateRadii(undoState, projectId, mappedNodes.radii);
...
CATMAID_UpdateConfidenceCommand = CATMAID_makeCommand(function(
  ...
  var exec = function(done, command, map) {
    var mNode = map.getWithTime(map.NODE, umNode[0], umNode[1], command);
    var execState = new CATMAID_LocalState([mNode.value, mNode.timestamp]);
    var updateConfidence = CATMAID_Nodes.updateConfidence(execState,
    ...
    return updateConfidence.then(function(result) {
      ...
      if (toConnector) {
        ...
        if (1 === updatedNodeIds.length) {
          ...
          throw new CATMAID_ValueError("Didn't expect more than one updated node for confidence update");
        }
      }
  ...
  var undo = function(done, command, map) {
    ...
    if (toConnector) {
      ...
      if (undefined === oldConfidence) {
        throw new CATMAID_ValueError("Can't undo confidence update, missing " +
            "history information");
      }
    }

    var undoState = new CATMAID_LocalState([mNode.value, mNode.timestamp]);
    var updateConfidence = CATMAID_Nodes.updateConfidence(undoState,
...
CATMAID_RemoveNodeCommand = CATMAID_makeCommand(function(
  ...
  var exec = function(done, command, map) {
    ...
    var removeNode = CATMAID_Nodes.remove(execState, projectId, mNode.value);
  ...
  var undo = function(done, command, map) {
    ...
    if (0 === mChildren.length) {
      var undoState = new CATMAID_LocalState(undefined, [mParentId, mParentEditTime]);
      create = CATMAID_Nodes.create(undoState, projectId, x, y, z,
          mParentId, radius, confidence, undefined, undefined, mLinks);
    } else {
      var parentEditionTime = "";
      var undoState = new CATMAID_LocalState([mParentId, mParentEditTime], undefined,
      ...
      create = CATMAID_Nodes.insert(undoState, projectId, x, y, z, mParentId,
...
CATMAID_CreateNodeCommand = CATMAID_makeCommand(function(
  ...
  var exec = function(done, command, map) {
    ...
    var execState = new CATMAID_LocalState(null, [mParent.value, mParent.timestamp]);

    // Create node, error handling has to be done by caller
    var create = CATMAID_Nodes.create(execState, projectId, x, y, z,
  ...
  var undo = function(done, command, map) {
    ...
    var removeNode = CATMAID_Nodes.remove(undoState, projectId, mNode.value);
...
CATMAID_InsertNodeCommand = CATMAID_makeCommand(function(
  ...
  var exec = function(done, command, map) {
    ...
    var execState = new CATMAID_LocalState([mParent.value, mParent.timestamp],
        null, [[mChild.value, mChild.timestamp]]);
    var insert = CATMAID_Nodes.insert(execState, projectId, x, y, z,
...
CATMAID_UpdateNodesCommand = CATMAID_makeCommand(
  ...
  var exec = function(done, command, map) {
    ...
    var execState = new CATMAID_SimpleSetState(state);
    var update = CATMAID_Nodes.update(execState, projectId, mTreenodes, mConnectors);
  ...
  var undo = function(done, command, map) {
    ...
    // Create a new mapped local state and call model function, which will
    // create multi node state for all treenodes and connectors.
    var undoState = new CATMAID_SimpleSetState(state);
    var update = CATMAID_Nodes.update(undoState, projectId, mTreenodes, mConnectors);
    return update.then(function(result) {
      var updatedTreenodes = result.old_treenodes;
      var updatedConnectors = result.old_connectors;
      // Map updated nodes forward
