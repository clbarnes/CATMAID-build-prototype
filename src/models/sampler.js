...
Sampling.domainArborFromModel = function(arbor, domain, strict) {
  return CATMAID_Sampling.domainArbor(arbor, domain.start_node_id,
...
Sampling.updateIntervalMap = function(arbor, intervals, targetEdgeMap,
  ...
  for (var i=0, imax=intervals.length; i<imax; ++i) {
    ...
    while (true) {
      ...
      if (!currentNodeId) {
        throw new CATMAID_ValueError("Arrived at root node without finding interval start");
      }
    }
  }
};
...
Sampling.intervalsFromModels = function(arbor, positions, domainDetails,
    ...
    strict) {
  if (!intervalLength) {
    throw new CATMAID_ValueError("Need interval length for interval creation");
  }

  // Get domain arbor, which is then split into slabs, which are then
  // further split into intervals of respective length.
  var domainArbor = CATMAID_Sampling.domainArborFromModel(arbor, domainDetails, strict);

  preferSmallerError = preferSmallerError === undefined ? true : !!preferSmallerError;
  createNewNodes = CATMAID_tools.getDefined(createNewNodes, false);
  ...
  for (var i=0; i<partitions.length; ++i) {
    ...
    for (var j=partition.length - 2; j>=0; --j) {
      ...
      if (distance < 0.0001) {
        ...
        if (!selectedNode) {
          ...
          if (createNewNodes && !isBlacklisted) {
            ...
            if (arbor.edges[newPointId]) {
              throw new CATMAID_PreConditionError("The temporary ID for the " +
        ...
        if (!selectedNode) {
          throw new CATMAID_ValueError("Could not select node for interval creation");
        }
...
Sampling.samplerEdges = function(arbor, sampler, target, allowedDomains) {
  ...
  var edges = sampler.domains.reduce(function(o, d) {
    ...
    var domainArbor = CATMAID_Sampling.domainArborFromModel(arbor, d);
...
Sampling.NodeProviders = {
  'active': function(arborParser) {
    ...
    return new Promise(function(resolve, reject) {
      if (!activeNodeId) {
        throw new CATMAID_ValueError("No node selected");
      }
      ...
      if (!arborParser.arbor.contains(activeNodeId)) {
        throw new CATMAID_ValueError("Active node not part of specified skeleton");
      }
  ...
  'select': function(arborParser) {
    return new Promise(function(resolve, reject) {
      var dialog = new CATMAID_OptionsDialog("Select node", {
        'Use active node': function() {
          ...
          CATMAID_msg("Selected node", activeNodeId);
  ...
  'root': function(arborParser) {
    return new Promise(function(resolve, reject) {
      ...
      if (!arbor.root) {
        throw new CATMAID_ValueError("No root node found");
      }
      resolve([arbor.root]);
    });
  },
  'tag': function(arborParser) {
    return new Promise(function(resolve, reject) {
      var tagInput = null;
      var dialog = new CATMAID_OptionsDialog("Provide tag", {
        'Use tagged nodes': function() {
          var tag = tagInput.value;
          var taggedNodes = arborParser.tags[tag];
          if (!taggedNodes) {
            throw new CATMAID_ValueError("No nodes found with tag \"" + tag + "\"");
          }
  ...
  'downstream': function(arborParser, options) {
    ...
    if (!referenceNodes) {
      throw new CATMAID_ValueError("At least one reference node is required");
    }
    return new Promise(function(resolve, reject) {
      var nodes = [];
...
Sampling.getArbor = function(skeletonId) {
  ...
  return CATMAID_fetch(project.id + '/' + skeletonId + '/1/1/1/compact-arbor', 'POST')
...
Sampling.DomainFactories = {
  ...
  'regular': {
     ...
     */
    makeDomains: function(skeletonId, options) {
      var startNodeProvider = CATMAID_Sampling.NodeProviders[options.domainStartNodeType];
      var endNodeProvider = CATMAID_Sampling.NodeProviders[options.domainEndNodeType];
      ...
      return Sampling.getArbor(skeletonId)
        ...
        .then(function(results) {
          ...
          var domains = domainStartNodeIds.reduce(function(o, startNodeId) {
            ...
            var domainArbor = CATMAID_Sampling.domainArbor(arbor, startNodeId,
                domainEndNodeIds);
...
Sampling.getIntervalBackboneNodes = function(arbor, startNodeId, endNodeId,
  ...
  if (intervalNodes[intervalNodes.length - 1] == startNodeId) {
    ...
    return strict ? null : CATMAID_Sampling.getIntervalBackboneNodes(arbor,
...
Sampling.getIntervalNodes = function(arbor, startNodeId, endNodeId, boundaryNodes) {
  ...
  var intervalBackbone = CATMAID_Sampling.getIntervalBackboneNodes(arbor,
      startNodeId, endNodeId, true);

  if (!intervalBackbone || intervalBackbone.length === 0) {
    throw new CATMAID_ValueError("Could not find interval backbone for between nodes " +
...
Sampling.updateSampler = function(projectId, samplerId, newProperties) {
  return CATMAID_fetch(projectId + '/samplers/' + samplerId + '/', 'POST', {
      'leaf_handling_mode': newProperties.leafHandlingMode,
    })
    .then(function(sampler) {
      CATMAID_Sampling.trigger(CATMAID_Sampling.EVENT_SAMPLER_UPDATED, samplerId);
      return sampler;
    });
};
...
Sampling.updateLeafHandlingMode = function(projectId, samplerId, leafHandling) {
  if (leafHandling !== 'short-interval') {
    return Promise.reject(new CATMAID_ValueError("Can only transform to short interval mode"));
  }
  let intervalsNeedingUpdate = 0;
  let intervalsUpdated = 0;
  return CATMAID_Sampling.getSampler(projectId, samplerId, true, true)
      .then(function(sampler) {
        if (sampler.leaf_segment_handling !== 'ignore') {
          throw new CATMAID_ValueError("Current leaf handling mode needs to be 'ignore'");
        }

        return Promise.all([
          sampler,
          CATMAID_fetch(project.id + '/' + sampler.skeleton_id + '/1/1/1/compact-arbor'),
        ]);
      })
      .then(function(results) {
        let sampler = results[0];
        let skeletonData = results[1];
        let arborParser = new CATMAID_ArborParser().init('compact-skeleton', skeletonData);
        ...
        for (let domain of sampler.domains) {
          ...
          let fragmentInfo = CATMAID_Sampling.getIgnoredFragments(projectId,
          ...
          for (let ignoredFragment of fragmentInfo.intervalFragments) {
            ...
            let intervalMap = {};
            let intervalInfo = CATMAID_Sampling.intervalsFromModels(
          ...
          if (intervalInfos.length === 0) {
            CATMAID_warn("No new intervals could be found in domain " +
                domain.id);
          } else {
      ...
      .then(function(samplerInfo) {
        ...
        return new Promise(function(resolve, reject) {
          ...
          var models = {};
          models[skeletonId] = new CATMAID_SkeletonModel(skeletonId);

          // Create virtual skeletons
          let arborParsers = new Map([[skeletonId, workParser]]);
          let nodeProvider = new CATMAID_ArborParserNodeProvider(arborParsers);
          ...
          var fakeSampler = CATMAID_tools.deepCopy(sampler);
            ...
            if (!dialogInfo) {
              ...
              glWidget.addSkeletons(models, function() {
                ...
                if (skeletonIds.length !== 1) {
                  throw new CATMAID_ValueError("Expected exactly one loaded skeleton");
                }
          ...
          for (let domainInfo of newIntervalsPerDomain) {
            ...
            let dialog = new CATMAID_Confirmation3dDialog({
            ...
            dialog.onOK = function() {
              CATMAID_fetch(project.id + '/samplers/domains/' +
                ...
                .then(function(result) {
                  ++intervalsUpdated;
                  showNextDialog();
                  CATMAID_msg("Success", intervals.length + " interval(s) created, using " +
                      result.n_added_nodes + " new node(s)");
                })
                .catch(reject);
            };
            dialog.onCancel = function() {
              CATMAID_msg("No intervals created", "Canceled by user");
              showNextDialog();
            };
      ...
      .then(function() {
        if (intervalsNeedingUpdate === intervalsUpdated) {
          ...
          return CATMAID_Sampling.updateSampler(projectId, samplerId, {
            'leafHandlingMode': newLeafHandling,
          });
        } else {
          CATMAID_warn("Could not update all domains that needed a leaf mode update");
        }
      });
};
...
Sampling.getIgnoredFragments = function(projectId, samplerId, arbor,
  ...
  var intervalMap = {};
  CATMAID_Sampling.updateIntervalMap(arbor, domainListIntervals,
      intervalMap, domain.start_node_id);

  // Get domain arbor
  let domainArbor = CATMAID_Sampling.domainArborFromModel(arbor, domain);
  ...
  while (workingSet.length > 0) {
    ...
    if (currentNodeId != domain.start_node_id &&
      ...
      if (!lastPos) {
        CATMAID_warn("Couldn't find position for last node " + fragment[i-1]);
      }
      if (!pos) {
        CATMAID_warn("Couldn't find position for current node " + fragment[i]);
      }
...
Sampling.getSampler = function(projectId, samplerId, withDomains,
    withIntervals) {
  return CATMAID_fetch(project.id + '/samplers/' + samplerId + '/', 'GET', {
...
CATMAID_asEventSource(Sampling);
