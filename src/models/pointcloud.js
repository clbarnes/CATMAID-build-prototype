...
Pointcloud.listAll = function(projectId, simple, with_images, order_by) {
  return CATMAID_fetch(projectId + '/pointclouds/', 'GET', {
    simple: !!simple,
...
Pointcloud.list = function(projectId, simple, with_images, pointcloudIds) {
  return CATMAID_fetch(projectId + '/pointclouds/', pointcloudIds ? 'POST' : 'GET', {
    simple: !!simple,
    with_images: !!with_images,
...
Pointcloud.add = function(projectId, name, points, description, images,
  ...
  return CATMAID_fetch(projectId + '/pointclouds/', 'PUT', data, undefined,
      undefined, undefined, undefined, {"Content-type" : null})
    .then(function(result) {
      CATMAID_Pointcloud.trigger(CATMAID_Pointcloud.EVENT_POINTCLOUD_ADDED, result.id);
      return result;
    });
};
...
Pointcloud.get = function(projectId, pointcloudId, withPoints, withImages, sampleRatio) {
  return CATMAID_fetch(projectId + '/pointclouds/' + pointcloudId + '/', 'GET', {
...
Pointcloud.delete = function(projectId, pointcloudId) {
  return CATMAID_fetch(projectId + '/pointclouds/' + pointcloudId + '/', 'DELETE')
    .then(function(result) {
      CATMAID_Pointcloud.trigger(CATMAID_Pointcloud.EVENT_POINTCLOUD_DELETED, result.id);
      return result;
    });
};
...
Pointcloud.getImagePath = function(projectId, pointcloudId, imageId) {
  return CATMAID_makeURL(project.id + '/pointclouds/' + pointcloudId +
      '/images/' + imageId + '/');
};
...
// Add events
CATMAID_asEventSource(Pointcloud);
