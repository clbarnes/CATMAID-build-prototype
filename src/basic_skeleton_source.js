...
var BasicSkeletonSource = function(name, options) {
  ...
  CATMAID_SkeletonSource.call(this, register);
  ...
  // part of a widget.
  this.owner = CATMAID_tools.getDefined(options.owner, null);
};

BasicSkeletonSource.prototype = Object.create(CATMAID_SkeletonSource.prototype);
...
BasicSkeletonSource.prototype.hasGroups = function() {
  return !CATMAID_tools.isEmpty(this.groups);
};
...
BasicSkeletonSource.prototype.clear = function(sourceChain) {
  ...
  this.orderedElements = [];
  this.groups = {};
  if (!CATMAID_tools.isEmpty(removedModels)) {
...
BasicSkeletonSource.prototype.removeSkeletons = function(skeletonIDs) {
  ...
  if (!CATMAID_tools.isEmpty(removed)) {
...
BasicSkeletonSource.prototype.updateModels = function(models, sourceChain) {
  ...
  if (!CATMAID_tools.isEmpty(newModels)) {
    this.append(newModels);
  }

  if (!CATMAID_tools.isEmpty(updatedModels)) {
 ...
 */
BasicSkeletonSource.prototype.handleAddedModels = CATMAID_noop;
BasicSkeletonSource.prototype.handleRemovedModels = CATMAID_noop;
BasicSkeletonSource.prototype.handleChangedModels = CATMAID_noop;
...
var checkGroupName = function(groups, name) {
  if (!name) {
    throw CATMAID_ValueError("Please give a valid group name");
  }
  if (groups[name]) {
    throw CATMAID_ValueError("The group '" + name + "' exists already");
  }
};
