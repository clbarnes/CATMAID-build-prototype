 ...
 * For example:
 *
 *     myModuleSettings = new CATMAID_Settings(
 *         'my-module',
 *         {
 *           version: 0,
 *           entries: {
...
function Settings(name, schema) {
  ...
  this.schema = schema;
  this.rendered = {};
  this.settingsStore = CATMAID_DataStoreManager.get(Settings.DATA_STORE_NAME);
  this._boundLoad = this.load.bind(this);
  this._storeThrottleTimeout = null;
  this.settingsStore.on(CATMAID_DataStore.EVENT_LOADED, this._boundLoad);
  this.load();
}
...
Settings.prototype.destroy = function () {
  this.settingsStore.off(CATMAID_DataStore.EVENT_LOADED, this._boundLoad);
};
  ...
  return this.settingsStore
      ...
      .then(function (stored) {
        ...
        return self.settingsStore
            ...
            .then(function () {
              CATMAID_msg('Success', 'User profile updated successfully.');
              self.load();
            });
      });
};
