...
CATMAID_asEventSource(SkeletonSource.prototype);
...
SkeletonSource.prototype.addSubscription = function(subscription, ignoreEmpty) {
  ...
  if (-1 !== index) {
    throw new CATMAID_SubscriptionError("Subscription already in use");
  }
  ...
  while (workingSet.length > 0) {
    ...
    if (seen.has(sub.source)) {
      throw new CATMAID_SubscriptionError("Cycles are not allowed when " +
...
SkeletonSource.prototype.removeSubscription = function(subscription) {
  ...
  if (-1 === index) {
    throw new CATMAID_ValueError("The subscription isn't part of this source");
  }
...
SkeletonSource.prototype.loadSubscriptions = function(ignoreEmpty) {
  ...
  for (var i=0, max=this.subscriptions.length; i<max; ++i) {
    ...
    if (SkeletonSource.UNION === op) {
      ...
      throw new CATMAID_ValueError("Unknown operation: " + op);
    }
  }
  ...
  // Update all others
  if (!ignoreEmpty || (result && !CATMAID_tools.isEmpty(result))) {
...
SkeletonSource.prototype.loadSource = function(silent) {
  ...
  if (0 === numModels) {
    if (!silent) {
      CATMAID_info('Selected source is empty.');
    }
    return false;
  }
...
SkeletonSource.prototype.appendSkeletons = function(skids) {
  this.append(skids.reduce(function(o, skid) {
    o[skid] = new CATMAID_SkeletonModel(skid, "", new THREE.Color(255, 255, 0));
    return o;
  }, {}));
};
...
SkeletonSource.prototype.annotate_skeleton_list = function() {
  CATMAID_annotate_neurons_of_skeletons(this.getSelectedSkeletons());
};

SkeletonSource.prototype.annotateSkeletonsWithName = function() {
  return CATMAID_annotateSkeletonsWithName(this.getSelectedSkeletons());
};
...
SkeletonSourceSubscription.prototype.register = function(target, keepCache) {
  ...
  var allEvents = this.mode === CATMAID_SkeletonSourceSubscription.ALL_EVENTS;
  var onlyRemovals = this.mode === CATMAID_SkeletonSourceSubscription.ONLY_REMOVALS;
  var onlyAdditions = this.mode === CATMAID_SkeletonSourceSubscription.ONLY_ADDITIONS;
  var onlyUpdates = this.mode === CATMAID_SkeletonSourceSubscription.ONLY_UPDATES;
