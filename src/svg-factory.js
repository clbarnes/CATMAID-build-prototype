...
var SVGFactory = function(width, height, viewX, viewY, viewWidth, viewHeight) {
  if (!width) {
    throw new CATMAID_ValueError("SVG needs valid width");
  }

  if (!height) {
    throw new CATMAID_ValueError("SVG needs valid height");
  }
  ...
  switch (options.edgeType) {
    ...
    case 'multibezier':
      ...
      if (options.controlPoints.length === 2) {
        ...
        throw new CATMAID_ValueError('More than two control points per line are not supported');
      }
    ...
    if (shrinkers[options.arrow]) {
      ...
      if (options.arrowUnit === 'userSpaceOnUse') {
        ...
        if (!options.strokeWidth) {
          throw new CATMAID_ValueError('Need strokeWidth option to calculate ' +
