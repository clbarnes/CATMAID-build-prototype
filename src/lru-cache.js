...
function MemoryAwareLRUCache(capacity, lifetime, maxMemoryFillRate) {
  if (!window.performance.memory) {
    throw new CATMAID_PreConditionError("Need window.performance.memory " +
        "extension, available e.g. in Chrome/Chromium");
  }
  LRUCache.call(this, capacity, lifetime);
  this.maxMemoryFillRate = CATMAID_tools.getDefined(maxMemoryFillRate, 1.0);
}
...
export let CATMAID_CacheBuilder = {
  makeMemoryAwareLRUCache: function(capacity, lifetime, maxMemoryFillRate, fallback) {
    ...
    try {
      ...
      if (fallback && error instanceof CATMAID_PreConditionError) {
