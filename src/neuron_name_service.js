  ...
  function init(empty) {
    ...
    return {
      ...
      loadConfigurationFromSettings: function (scope, sync) {
        if (!(scope in CATMAID_Settings.SCOPES))
          scope = 'session';
        if (!CATMAID_NeuronNameService.Settings) {
          CATMAID_NeuronNameService.Settings = new CATMAID_Settings(
              'neuron-name-service',
              {
                version: 0,
                entries: {
        ...
        var loadValues = (function () {
          componentList = $.extend(true, [], CATMAID_NeuronNameService.Settings[scope].component_list);
          formatString = CATMAID_NeuronNameService.Settings[scope].format_string;
          autoTrimEmptyValues = CATMAID_NeuronNameService.Settings[scope].auto_trim_empty;
          removeDuplicates = CATMAID_NeuronNameService.Settings[scope].remove_neighboring_duplicates;
          this.refresh();
        }).bind(this);
        ...
        if (sync) {
          ...
          return CATMAID_NeuronNameService.Settings
              .load()
              .then(loadValues);
        }
      },
      ...
      {
        if (!client) {
          throw new CATMAID_ValueError("Please provide a valid client");
        }
        ...
        for (var skid in models) {
          ...
          if (!id) throw new CATMAID_ValueError("Please provide valid IDs");
      ...
      updateNames: function(skids, callback)
      {
        ...
        var update = function(skidsToUpdate, data, resolve, reject) {
          var name = function(skid) {
            ...
            var componentValues = componentList.map(function (l) {
              if ('neuronname' === l.id) {
                ...
                if (skid in data.skeletons) {
                  ...
                  // annotation.
                  var label = metaLabel(CATMAID_annotations.getID(l.option));
                  if (null !== label) {
                    return [label];
                  }
                }
                ...
                if (skid in data.skeletons) {
                  ...
                  var oa = data.skeletons[skid].annotations.reduce(function(o, a) {
                    if (a.uid === CATMAID_session.userid) {
                      o.push(data.annotations[a.id]);
                    }
                    return o;
                  }, []);
            ...
            var removeDuplicates = CATMAID_NeuronNameService.Settings.session.remove_neighboring_duplicates;
            var autoTrim = CATMAID_NeuronNameService.Settings.session.auto_trim_empty;
