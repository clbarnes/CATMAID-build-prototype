...
var Annotations = {
  ...
  forSkeleton: function(projectId, skeletonId) {
    ...
    return CATMAID_fetch(url, 'POST', params).then(function(json) {
      if (json.annotations && json.annotations instanceof Array) {
        ...
        throw new CATMAID_Error('Can\'t load annotations', json);
      }
    });
  },
  ...
  forSkeletons: function(projectId, skeletonIds) {
    ...
    if (validSkeletonIds.length !== skeletonIds.length) {
      throw new CATMAID_ValueError("No skeleton ID can be null or undefiend");
    }
    return CATMAID_fetch(projectId + '/annotations/forskeletons', 'POST', {
  ...
  forTarget: function(projectId, entityIds) {
    return CATMAID_fetch(project.id + '/annotations/query', 'POST', {
  ...
  forAny: function(projectId, entityIds, skeletonIds, sort) {
    ...
    if (entityIds && entityIds.length > 0) {
      requestAnnotations = requestAnnotations.then(function(ann) {
        return CATMAID_Annotations.forTarget(project.id, entityIds)
    ...
    if (skeletonIds && skeletonIds.length > 0) {
      requestAnnotations = requestAnnotations.then(function(ann) {
        return CATMAID_Annotations.forSkeletons(project.id, skeletonIds)
          .then(function(annotationInfo) {
            return collectAnnotations(ann, annotationInfo);
          });
      });
    ...
    return CATMAID_fetch(url, 'POST', params).then(function(json) {
      if (json.deleted_annotations) {
        ...
        if (deletedAnnotationIds.length > 0) {
          CATMAID_Annotations.trigger(CATMAID_Annotations.EVENT_ANNOTATIONS_DELETED,
        ...
        if (changedEntities.length > 0) {
          ...
          CATMAID_Annotations.trigger(CATMAID_Annotations.EVENT_ANNOTATIONS_CHANGED,
              changedEntities, []);
        }
      }
  ...
  add: function(projectId, entityIds, skeletonIds, annotations, metaAnnotations) {
    ...
    return CATMAID_fetch(projectId + '/annotations/add', 'POST', data)
      .then(function(e) {
  ...
  addMissingNeuronNames: function(projectId, skeletonIds) {
    return CATMAID_fetch(projectId + '/annotations/add-neuron-names', 'POST', {
...
export let CATMAID_AddAnnotationsCommand = CATMAID_makeCommand(function(projectId,
  ...
  var exec = function(done, command) {
    var addAnnotations = CATMAID_Annotations.add(projectId, entityIds, skeletonIds,
  ...
  var undo = function(done, command) {
    ...
    if (undefined === command._createdLinks) {
      throw new CATMAID_ValueError('Can\'t undo creation of annotation, history data not available');
    }
    ...
    var promises = command._createdLinks.map(function(annotation) {
      return CATMAID_Annotations.remove(projectId, annotation.entities, [annotation.id]);
    });
...
export let CATMAID_RemoveAnnotationsCommand = CATMAID_makeCommand(function(projectId,
  ...
  var annotationMap = annotationIds.reduce(function(o, annotationId) {
    o[annotationId] = CATMAID_annotations.getName(annotationId);
    return o;
  }, {});

  var exec = function(done, command) {
    var removeAnnotations = CATMAID_Annotations.remove(projectId, targetIds,
        annotationIds);
  ...
  var undo = function(done, command) {
    ...
    if (undefined === command._removed_annotations) {
      throw new CATMAID_ValueError('Can\'t undo removal of annotation, history data not available');
    }
    ...
    var promises = Object.keys(remAnnotationIds).map(function(annotationId) {
      ...
      if (!annotation) {
        throw new CATMAID_ValueError("Can't undo removal of annotation, can't find annotation name for ID " + annotationId);
      }
      return CATMAID_Annotations.add(projectId, this[annotationId].targetIds, null,
          [annotation]);
    }, remAnnotationIds);
    return Promise.all(promises).then(done);
  };
