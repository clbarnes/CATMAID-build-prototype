...
import {SynapseClustering} from "@catmaid/arbor";
 ...
 * @param {number}  mergeMode (optional) Either CATMAID_UNION (default) or
 *                            CATMAID_INTERSECTION
...
function executeNodeFilterRules(rules, nodeIndex, inputMap, mapResultNode) {
  ...
  rules.forEach(function(rule, i) {
    ...
    // with anything.
    var mergeMode = i === 0 ? CATMAID_UNION : rule.mergeMode;
    ...
    if (rule.invert && noOwnInversion &&
      ...
      CATMAID_warn(`Can't invert rule "${rule.name}", because it doesn't require arbor and arbor isn't available.`);
      return;
    }
    ...
    sourceNodeIds.forEach(function(nodeId) {
      ...
      if (rule.invert && noOwnInversion) {
        ...
        nodeCollection = CATMAID_SkeletonFilter.invert(nodeCollection,
            unfilteredNode);
      }
      // Merge all point sets for this rule. How this is done exactly (i.e.
      // OR or AND) is configured separately.
      if (nodeCollection && !CATMAID_tools.isEmpty(nodeCollection)) {
...
NodeFilter.prototype.execute = function(mapResultNode, keepInputCache) {
  var rules = CATMAID_NodeFilter.getActiveRules(this.rules);
...
NodeFilter.getActiveRules = function(rules) {
  if (!rules || 0 === rules.length) {
    rules = CATMAID_DefaultFilterRuleSets.get('node');
  }
 ...
 */
CATMAID_SkeletonFilter.fetchArbors = function(skeletonIds, needsArbor,
    needsPartners, needsTags, needsTime, target) {
  return new Promise(function(resolve, reject) {
    var nns = CATMAID_NeuronNameService.getInstance();
    var arbors = target || {};
    fetchSkeletons(
      skeletonIds,
      function(skid) {
        ...
        var t = needsTags ? "1" : "0";
        return CATMAID_makeURL(project.id + '/' + skid + '/' + a + '/' + p + '/' + t + '/compact-arbor');
      },
      function(skid) {
        return {
          with_time: needsTime
      ...
      function(skid, json) {
        ...
        if (needsArbor) {
          var ap = new CATMAID_ArborParser();
          ap.tree(json[0]);
...
CATMAID_SkeletonFilter.loadVolumes = function(volumeIds, target) {
  var volumePromises = volumeIds.map(function(v) {
    return CATMAID_Volumes.get(project.id, v)
      .then(function(volume) {
        var meshes = CATMAID_Volumes.x3dToMeshes(volume.mesh);
        if (meshes.length < 1) {
          throw new CATMAID_ValueError("Can't create mesh for volume " + volume.id);
        }
        if (meshes.length > 1) {
          throw new CATMAID_ValueError("Too many meshes for volume " + volume.id);
        }
        target[volume.id] = {
          intersector: CATMAID_Volumes.makeIntersector(meshes[0])
        };
      });
  });
...
function prepareFilterInput(skeletonIds, rules, input) {
  ...
  if (needsArbor || needsTags || needsPartners) {
    ...
    if (neededSkeletonIds.length > 0) {
      let fetchSkeletons = CATMAID_SkeletonFilter.fetchArbors(skeletonIds,
  ...
  if (neededInput.has("volume")) {
    ...
    prepareActions.push(CATMAID_SkeletonFilter.loadVolumes(volumeIds, input.volumes));
  }
      ...
      if (domainId !== undefined) {
        domainRetrieval = domainRetrieval
          .then(function(interval) {
            return CATMAID_fetch(project.id + '/samplers/domains/' +
                domainId + '/details');
          })
    ...
    for (var i=0; i<nRules; ++i) {
      ...
      if (intervalId !== undefined) {
        intervalRetrieval = intervalRetrieval
          .then(function() {
            return CATMAID_fetch(project.id  + '/samplers/domains/intervals/' +
                intervalId + '/details');
          })
          .then(function(interval) {
            return CATMAID_fetch(project.id + '/samplers/domains/' +
...
function mergeNodeCollections(filterTargetId, other, mergeMode, mapNode, stats) {
  ...
  if (CATMAID_UNION === mergeMode) {
  ...
  } else if (CATMAID_INTERSECTION === mergeMode) {
    ...
    throw new CATMAID_ValueError("Unknown merge mode: " + mergeMode);
  }
 ...
 * @param {Object} skeletonArbors  Maps skeleton IDs to arbor info objects
 */
CATMAID_SkeletonFilter.prototype.execute = function(mapResultNode, keepInputCache) {
  var rules = CATMAID_SkeletonFilter.getActiveRules(this.rules, this.skeletonIndex);
 ...
 */
CATMAID_SkeletonFilter.prototype.getNodeLocations = function(nodeCollection, addRadiusPoints, nNodes) {
  ...
  for (var nodeId in nodeCollection) {
    ...
    if (addRadiusPoints && radius && radius > 0) {
      var radiusPoints = CATMAID_getIcoSpherePoints(p[0], p[1], p[2], radius);
...
CATMAID_SkeletonFilter.getActiveRules = function(rules, skeletonIndex) {
  var nns = CATMAID_NeuronNameService.getInstance();

  if (!rules || 0 === rules.length) {
    rules = CATMAID_DefaultFilterRuleSets.get('skeleton');
  }
 ...
 * collection.
 */
CATMAID_SkeletonFilter.invert = function(nodeCollection, arbor) {
...
export let CATMAID_NodeFilterSettingFactories = {
  ...
  'volume': function(container, options) {
    ...
    // DOM location
    var volumeSelectionWrapper = CATMAID_createVolumeSelector({
      mode: "radio",
...
export let CATMAID_SkeletonFilterStrategy = {
    ...
    filter: function(skeletonId, neuron, input, options, invert) {
      ...
      if (options.expected !== undefined) {
        if (!cuts || (options.expected && cuts.length !== options.expected)) {
          CATMAID_warn(`Cannot extract sub-arbors for "${neuron.name}", wrong number of tag cuts: ${cuts}`);
          return {};
        }
      }
  ...
  "synaptic": {
    ...
    filter: function(skeletonId, neuron, input, options, invert) {
      ...
      if ('pre' === options.relation) {
        ...
        selectedPartners = CATMAID_tools.deepCopy(partners[0]);
        ...
        throw new CATMAID_ValuError("Unsupported relation: " + options.relation);
      }
  ...
  'strahler': {
    ...
    filter: function(skeletonId, neuron, input, options, invert) {
      ...
      var value = CATMAID_tools.getDefined(options.strahlerValue, 2);
      var op = CATMAID_tools.getDefined(options.op, "greater");
      var includedNodes = {};
      if (op === "smaller") {
        ...
        throw new CATMAID_ValueError("Unknown strahler filter operation: " + op);
      }
  ...
  'sampler-domain': {
    ...
    filter: function(skeletonId, neuron, input, options, invert) {
      ...
      let includedNodes = {};
      try {
        let domainArbor = CATMAID_Sampling.domainArbor(arbor, domain.start_node_id,
  ...
  'sampler-interval': {
    ...
    filter: function(skeletonId, neuron, input, options, invert) {
      ...
      var includedNodes = {};
      try {
        var intervalNodes = CATMAID_Sampling.getIntervalNodes(arbor,
...
export let CATMAID_SkeletonFilterSettingFactories = {
  ...
  'endnodes': function(container, options) {
    ...
    var $includeRoot = CATMAID_DOM.createCheckboxSetting(
  ...
  'tags': function(container, options) {
    var $tag = CATMAID_DOM.createInputSetting("Tag", "",
  ...
  'subarbor': function(container, options) {
    ...
    var $expected = CATMAID_DOM.createInputSetting("Expected", "",
  ...
  'single-region': function(container, options) {
    var $tagStart = CATMAID_DOM.createInputSetting("Start tag", "",
    ...
    var $tagEnd = CATMAID_DOM.createInputSetting("End tag", "",
  ...
  'binary-split': function(container, options) {
    ...
    var $region = CATMAID_DOM.createSelectSetting("Region",
  ...
  'synaptic': function(container, options) {
    ...
    var $otherNeurons = CATMAID_DOM.createSelectSetting("Source of synaptic neurons",
    ...
    var $relation = CATMAID_DOM.createSelectSetting("Relation of base set to above partners",
  ...
  'volume': function(container, options) {
    ...
    var initVolumeList = function() {
      return CATMAID_Volumes.listAll(project.id).then(function(json) {
          var volumes = json.sort(function(a, b) {
            return CATMAID_tools.compareStrings(a.name, b.name);
          }).map(function(volume) {
            return {
              title: volume.name,
              value: volume.id
          ...
          var node = CATMAID_DOM.createRadioSelect('Volumes', volumes,
              selectedVolume, true);
  ...
  'users': function(container, options) {
    ...
    var $userField = CATMAID_DOM.createInputSetting("Node creator", "",
  ...
  'strahler': function(container, options) {
    ...
    var $tag = CATMAID_DOM.createNumericInputSetting("Strahler", "2", 1,
  ...
  'in-skeleton-source': function(container, options) {
    ...
    var $otherNeurons = CATMAID_DOM.createSelectSetting("Source",
